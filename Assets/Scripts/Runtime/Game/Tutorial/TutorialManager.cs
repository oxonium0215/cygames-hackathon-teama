...
namespace Game.Tutorial
{
    public class TutorialManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // Configure auto-sizing and disable word wrapping for the dialogue text
            if (dialogueText != null)
            {
                dialogueText.enableAutoSizing = true;
                dialogueText.textWrappingMode = TextWrappingModes.NoWrap; // Use this instead of enableWordWrapping
                dialogueText.fontSizeMin = 18;
                dialogueText.fontSizeMax = 25;
            } else
            {
                Debug.LogError("DialogueText is not assigned in the TutorialManager Inspector!");
            }
        }

        private void Update()
        {
            if (IsTutorialActive && !isEndingTutorial && canAdvance && jumpAction != null && jumpAction.WasPressedThisFrame())
            {
                AdvanceStep();
            }
        }
        ...
        public void StartTutorial(List<TutorialStep> stepsToRun)
        {
            ...
            StartCoroutine(EnableAdvancementAfterDelay(0.1f)); // Add a small delay
            ProcessStep(currentStepIndex);
        }

        private IEnumerator EnableAdvancementAfterDelay(float delay)
        {
            canAdvance = false;
        ...
        private IEnumerator EndTutorialRoutine()
        {
            ...
            {
                screenOverlay.gameObject.SetActive(false);
                yield return StartCoroutine(ReturnCameraToOrigin(0.5f, AnimationCurve.EaseInOut(0, 0, 1, 1)));
            }
